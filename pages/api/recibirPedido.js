  ...
  if (method === 'GET') {
    ...
    try {
      ...
      const origenResult = await pool.request()
        .input('CIDALMACEN', sql.Int, pedido.Origen)
        .query('SELECT CNOMBREALMACEN FROM admAlmacenes WHERE CIDALMACEN = @CIDALMACEN');

      const destinoResult = await pool.request()
        .input('CIDALMACEN', sql.Int, pedido.Destino)
        .query('SELECT CNOMBREALMACEN FROM admAlmacenes WHERE CIDALMACEN = @CIDALMACEN');
      ...
      // Obtener nombres de los productos desde admProductos
      const productosConNombre = await Promise.all(
        productos.map(async (producto) => {
          const productoInfo = await pool.request()
            ...
            .query('SELECT CNOMBREPRODUCTO FROM admProductos WHERE CIDPRODUCTO = @CIDPRODUCTO');

          return {
            nombre: productoInfo.recordset[0]?.CNOMBREPRODUCTO || 'Producto no encontrado',
            cantidad: producto.Unidades,
            codigo: producto.CIDPRODUCTO
          };
        })
      );
